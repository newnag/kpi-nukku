                ...
                <form method="POST" action="{{ route('sar_reports.update', $report->id) }}" class="space-y-6">
                    ...
                    <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <input type="text" name="title" id="title" class="w-full border rounded px-3 py-2"
                            ...
                            placeholder="เช่น รายงานการประเมินตนเอง ประจำปี 2566">
                    </div>
                    {{-- ส่วนที่ 1 --}}
                    <div class="bg-white shadow rounded-lg p-6">
                ...
                ">
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <div x-data="{
                                open: false,
                                style: '',
                                toggle() { this.open = !this.open; if (this.open) this.update() },
                                ...
                                update() {
                                    this.$nextTick(() => {
                                        ...
                                        const menuHeight = menu ? (menu.offsetHeight || 0) : 0;
                                        // Compute horizontal position (align right with some clamping)
                            ...
                            @resize.window="if (open) update()"
                            @scroll.window.passive="if (open) update()">
                            <!-- ปุ่มหลัก -->
                            ...
                            <!-- เมนูย่อย (teleport เพื่อกันโดน container บัง) -->
                            <template x-teleport="body">
                                <div x-show="open" x-ref="menu" @click.outside="close" @keydown.escape.window="close"
                                    class="fixed w-52 bg-white border border-gray-300 rounded-md shadow-lg z-[9999]"
                                    x-cloak
                                    :style="style">
                                    ...
                                    <!-- Excel -->
                                    <a id="exportExell" href="{{ route('sar_reports.export.xlsx', $report->id) }}"
                                        target="_blank"
                                        class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition">
                                        <i data-lucide="file-spreadsheet" class="w-4 h-4 mr-2 text-green-700"></i> Excel
                                        (XLSX)
                                    </a>
