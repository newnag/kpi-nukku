...
@section('content')
                    ...
                    <div class="charts-of-standard" data-standard-id="{{ $standard->id }}"
                        ...
                        @foreach ($bucket['indicators'] as $i => $c)
                            <div class="chart-card" data-standard="{{ $standard->id }}"
                                ...
                                data-index="{{ $i }}"
                                style="{{ $i >= 5 ? 'display:none;' : '' }};background:#fff;border-radius:16px;padding:16px;box-shadow:0 2px 10px rgba(0,0,0,.06);">
    ...
    <script>
        (function() {
                ...
                // ====== สร้างกราฟ + เก็บค่าเดิม ไว้กรองทีหลัง ======
                // key = canvasId -> Chart instance / original data
                const chartInstances = {};
                const chartOriginals = {};

                function initChartsFromInlineJSON() {
                    // เดินทุกแคนวาสที่ id เริ่มด้วย 'chart-'
                    const COLOR_BY_YEAR = {
                        '2020': '#8B5CF6', // ม่วง
                        '2021': '#FCA5A5', // แดงอ่อน
                        '2022': '#60A5FA', // ฟ้า
                        '2023': '#34D399', // เขียว
                        '2024': '#10B981', // เขียวเข้ม
                        '2025': '#EF4444', // แดง
                    };

                    // ฟังก์ชันเลือกสีตามปี
                    function getColorByYear(y) {
                        return COLOR_BY_YEAR[String(y)] || '#9CA3AF'; // ถ้าไม่เจอ → เทา
                    }
                    document.querySelectorAll('canvas[id^="chart-"]').forEach(cv => {
                        const canvasId = cv.id; // ex. chart-1-99
                        const key = canvasId.replace(/^chart-/, ''); // ex. 1-99
                        const dataEl = document.getElementById('data-' + key); // script#data-1-99
                        ...
                        const years = (payload.years || []).map(y => String(y)); // เก็บเป็น string ให้เทียบง่าย
                ...
                // ====== ปุ่ม "แสดงเพิ่มเติม" ของแต่ละมาตรฐาน ======
                function bindShowMore() {
                    document.querySelectorAll('.btn-show-more').forEach(btn => {
                                btn.addEventListener('click', function() {
                                            const sid = this.dataset.standardId;
                                            const container =
                                                document.querySelector(`.charts-of-standard[data-standard-id="${sid}"] `);
                                        if (!container) return;
                                  ...
                                  // ====== ฟิลเตอร์กราฟ (แสดง/ซ่อนการ์ด + กรองปีภายในกราฟ) ======
                                  function applyFilters() {
                                    const vYear = String($year.value || ''); // บังคับเป็น string
                                    const vCode = $code.value;
                                    const vStd  = $std.value;
                                    const vDim  = $dim.value;
                                    const vType = $type.value;

                                    // 1) แสดง/ซ่อนการ์ดตามเมทาดาต้า
                                    ...
                                    document.querySelectorAll('.charts-of-standard').forEach(group => {
                                      const anyVisible = !![...group.querySelectorAll('.chart-card')].find(c => c.style.display !== 'none');
                                    ...
                                    // 3) อัปเดตข้อมูลภายในกราฟตามปีที่เลือก
                                    Object.entries(chartInstances).forEach(([canvasId, inst]) => {
                                      const orig = chartOriginals[canvasId];
                                      if (!orig) return;

                                      if (vYear) {
                                        // เอาเฉพาะข้อมูลปีที่ตรงกัน
                                        const idxs = [];
                                        orig.years.forEach((y, i) => { if (String(y) === vYear) idxs.push(i); });
                                  ...
                                  function resetFilters() {
                                    [$year, $code, $std, $dim, $type].forEach(sel => { if (sel) sel.selectedIndex = 0; });
                                    ...
                                    // คืนค่ากราฟทั้งหมดจากค่าเดิม
                                    Object.entries(chartInstances).forEach(([canvasId, inst]) => {
                                      const orig = chartOriginals[canvasId] || { years: [], values: [] };
                                      inst.data.labels = orig.years.slice();
                                      inst.data.datasets[0].data = orig.values.slice();
                                      inst.update();
                                    });

                                    // ถ้าปุ่ม "แสดงเพิ่มเติม" เคยกดเป็น expanded ให้รีเซ็ตข้อความ
                                  ...
                                  document.getElementById('reset-filters').addEventListener('click', () => { resetFilters(); applyFilters(); });

                                  // ====== เริ่มต้น ======
                                  // 1) สร้างกราฟทั้งหมดจาก <script type="application/json" id="data-...">
        initChartsFromInlineJSON();
  // 2) Bind "แสดงเพิ่มเติม"
      bindShowMore();
  // 3) (ถ้าอยากให้เห็นทั้งหมดตั้งต้นก็ไม่ต้องทำอะไรเพิ่ม)
    })();
</script>
        ...
        (function() {
            ...
            // อ่าน payload จาก <script id="data-...">
            function snapshotOriginals() {
                document.querySelectorAll('canvas[id^="chart-"]').forEach(cv => {
                    const id = cv.id; // chart-<standardId>-<indicatorId>
                    const dataEl = document.getElementById(id.replace('chart-', 'data-'));
                    if (!dataEl) return;
                    const payload = parseJSONSafe(dataEl.textContent || '{}', {});
            ...
            // ===== helpers =====
            // เอาอินสแตนซ์ของกราฟที่สร้างไว้แล้ว (Chart.js v3 ขึ้นไป)
            ...
            // อัปเดตกราฟทุกใบตามปีที่เลือก
            function updateChartsForYear(selectedYear) {
                document.querySelectorAll('canvas[id^="chart-"]').forEach(cv => {
                    const id = cv.id;
                    const chart = getChartInstance(cv);
                    const orig = originals.get(id);
                    if (!chart || !orig) return;

                    if (selectedYear) {
                        const want = String(selectedYear);
                        const idxs = [];
                        orig.years.forEach((y, i) => {
                            if (String(y) === want) idxs.push(i);
                        });
                        chart.data.labels = idxs.map(i => orig.years[i]);
                        chart.data.datasets[0].data = idxs.map(i => orig.values[i]);
                    } else {
                        chart.data.labels = [...orig.years];
                        chart.data.datasets[0].data = [...orig.values];
                    }
                    chart.update();
                });
            }

            // ===== ปุ่มกรองข้อมูล: เหลือเฉพาะปีที่เลือก =====
            if (btnApply) {
                btnApply.addEventListener('click', () => {
                    const vYear = (yearSel && yearSel.value) ? String(yearSel.value) : '';
                    updateChartsForYear(vYear || null);

                    // ซ่อน/แสดงการ์ดตามปี (ใช้ data-years เดิมของคุณ)
                    document.querySelectorAll('.chart-card').forEach(card => {
                        if (!vYear) {
                            card.style.display = '';
                            return;
                        }
                        const years = parseJSONSafe(card.getAttribute('data-years') || '[]', []).map(
                            String);
                        card.style.display = years.includes(String(vYear)) ? '' : 'none';
                    });

                    // ซ่อน/โชว์หัวข้อของมาตรฐาน
                    document.querySelectorAll('.charts-of-standard').forEach(group => {
                        const anyVisible = !![...group.querySelectorAll('.chart-card')]
            ...
            // ===== ปุ่มล้างค่า: คืนทุกอย่างเหมือนเดิม =====
            if (btnReset) {
                btnReset.addEventListener('click', () => {
                    if (yearSel) yearSel.selectedIndex = 0;
                    updateChartsForYear(null); // คืน labels/data เดิมทุกใบ

                    document.querySelectorAll('.chart-card').forEach(c => c.style.display = '');
                    document.querySelectorAll('.charts-of-standard, .stat-title').forEach(el => el.style
                        .display = '');
                });
            }
        })();
