...
<div x-data="{
    ...
    add() {
        ...
        if (!name) return;
        const v = { id: Date.now() + Math.random(), name, type: this.newType, value: this.newType === 'defined' ? (this.newValue ?? '') : '' };
    ...
    <div class="grid grid-cols-1 md:grid-cols-[1fr,260px,auto] gap-3 items-center">
        ...
        <input x-ref="newName" x-model="newName" type="text" placeholder="กรุณาตั้งชื่อตัวแปร"
            class="p-2 w-full bg-white rounded-xl border border-slate-300 
    ...
    <div class="space-y-3">
        ...
        <textarea x-ref="formula" x-model="formula" rows="5"
            class="w-full bg-white rounded-2xl border border-blue-200 focus:border-blue-500 focus:ring-blue-500 p-3 text-sm md:text-base"></textarea>
